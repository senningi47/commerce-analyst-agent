...
### 2.39 Codex 二轮复核处置轮（Claude Code，2026-09-18，零付费；复核报告 `docs/reviews/codex-review-verification.md` 保持未跟踪）

Codex 独立复核一轮回应：8 项 FIXED-VERIFIED、2 项 FIXED-DISPUTED（设计可留、实现有错）、F8 NOT-FIXED（如实披露）；免费重评分 A 5/40、B 4/40、封闭 1/10 与一轮更正口径一致；并指出 **4 项 P1**（评分器 ordered/unordered 两个真实缺陷、嵌套 star 绕过身份保护、文档「全面更正」未落地正文）。逐项复现核实后全部属实，处置：

1. **评分器重写（P1-1/P1-2）**：ordered 分支误用 `pairwise` 比较相邻行（单行必真/正确多行判错）→ 参考行 i 对实际行 i 逐单元格比较；unordered 分支先按行位置配对（文本/NULL 重排必错）→ 两侧按参考列类型独立归一化后再做多重集比较；尾部死代码清除；13 条边界回归。
2. **策略硬化（P1-3）**：表达式内嵌套 star（`(s.*)`/`COALESCE(s.*,s.*)`/`COUNT(s.*)`）一律拒绝（`COUNT(*)` 豁免保留）；保守误拒如实披露（COUNT FILTER 命名身份列、CTE 派生计数别名身份名）。
3. **题集更正（P2-1/P2-3）**：row_order 逐题声明 10 题（新增 `descending`；dev-02/04、reg-02/03 原误标 unordered）；dev-15/reg-03 参考 SQL 改 `EXTRACT(EPOCH …)` 折算天数（原 interval 无法过产品序列化边界）；builder 升级三层验收（执行 + AST + QueryEngine 边界自评分 50/50）；题面与语义未变。
4. **文档口径落地（P1-4）**：最终报告正文/README/面试材料/HANDOFF 当前态全部改用更正后口径（0/643、12.5% vs 10.0%、混合口径估算、peak 收窄、932 测试、三层验证）；历史带日期记录保留原值；账本 7.93 来源标签更正（7.40 平台核对 + 0.53 agent 估算）。
5. **F8 登记**：HANDOFF §5 补具体排期项（持久 cursor 列迁移 + 晚到/跨 attempt/重连验收条件）；最终报告 §6 同步。
6. **免费重评分**（修复后评分器 + 新元数据重放保存 SQL）：A 5/7 执行正确（5/40）、B 4/4（4/40）、closed 1/1（1/10）——数字与一轮更正一致；dev-02 在新 ordered 契约下仍正确。
...
- **预算终账（2026-09-19 平台对账）**：**平台实扣 91.06 元 / 160 线（57%，含 sim 侧）**；收官混合估算 88.19 与实差 +2.87（≈3.3%：sim + 汇率漂移 + 小额未计轮，残差量级反向排除隐性 peak 计费），对账表见账本 `sim_reconciliation_20260919`；产品评测 $0.071519040；**预算口径从「估算」升级为「平台核对」，余 ~43%**。
...
- **待用户（均非阻塞）**：两轮审查处置的 commit 授权（§8.15/§8.16）；push 授权；Day 1–2 工件/4 份早期计划补录裁定；sim 侧余额核对；审查遗留 P2（SSE 持久游标列、导入器库级聚合）排期裁定。
...
- **项目终账**：agent 实测累计 ≈$7.94 ≈ 56.1 元（a 批 6.15 + 消融 1.67 + 产品 0.06 + 历史门控与验证）；平台核对口径累计 ≈88.1 元 / 160 线。〔2026-09-18 更正：88.1 为**混合口径估算** = 7.40 平台核对 + 0.53 agent 估算 + 现存遥测×7.07，sim 侧待核对，非平台实扣终账——原文「平台核对口径」标签有误。〕
...
### 8.16 二轮复核处置轮（2026-09-18，零付费，commit 待授权）
...
### 8.20 sim 对账轮（2026-09-19，零付费，免费对账裁定行使）

- **用户读数**：平台总消耗 **91.06 元**；分日 09-15 16:00–24:00 22.36 / 09-16 47.74 / 09-17 12.73（16:00 前累计 = 8.23）。
- **对账**：收官混合估算 88.19 + 已知未计入轮 ≈0.78（f/g/h、v4、午后诊断）→ agent 侧 ≈88.97；**残差 +2.09 元（2.3%）= sim 侧 + 汇率漂移 + 取整**；分窗指示性核对全一致（±1–2 元摆动，c 批跨午夜切分为估算）。
...
### 8.22 批次 2 裁定轮（2026-09-19，零付费）

- **用户裁定**：批次 2 历史材料**不执行**——specs/plans/research/早期报告/DATA_PROVENANCE/spikes 维持未跟踪；evaluator-only manifest 同状态（从未读取）；`.vscode/`、`.tmp-*` 永不入库。补录任务关闭（批次 1 已达成干净 checkout 可运行）。
- **后续安排裁定全链走完**：归档 ✅ → push ✅ → sim 对账 ✅ → 补录批次 1 ✅ → 批次 2 裁定不执行 ✅。交接文档无未决事项；剩余仅两项已排期 P2（F8、导入器库级聚合，§5.4/§5.5）与可选能力杠杆（§5.6/§5.7）。
- **零付费零 DB 变更零进程变更**。
