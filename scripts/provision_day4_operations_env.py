"""Provision ignored Day 4 operation credentials without printing secrets."""
...
def ensure_day4_operations_env(
    env_path: Path,
    token_factory: Callable[[], str] = lambda: secrets.token_urlsafe(32),
    key_factory: Callable[[], str] = lambda: secrets.token_urlsafe(48),
) -> bool:
    """Append Day 4 credentials through one same-directory atomic replacement."""

    if env_path.name != ".env":
        raise ValueError("Day 4 operation credentials may only be written to .env")
    ...
    if all(values.get(key) for key in _DAY4_KEYS):
        return False
    if any(key in values for key in _DAY4_KEYS):
        raise ValueError("Day 4 operation credential state is partial or empty")
    generated_passwords = [token_factory() for _spec in _PASSWORD_SPECS]
    generated_keys = [key_factory() for _key in _HMAC_KEYS]
    if not all(
        re.fullmatch(r"[A-Za-z0-9_-]+", value)
        for value in (*generated_passwords, *generated_keys)
    ):
        raise ValueError("generated Day 4 secrets must be URL-safe")
    generated_secrets = {*generated_passwords, *generated_keys}
    if len(generated_secrets) != len(generated_passwords) + len(generated_keys):
        raise ValueError("generated Day 4 secrets must be pairwise distinct")
    existing_secrets = {
        values[key]
        for key in _EXISTING_PRODUCT_SECRET_KEYS
        if values.get(key)
    }
    if generated_secrets & existing_secrets:
        raise ValueError("Day 4 secrets must be distinct from existing Product roles")
    lines: list[str] = []
    for spec, password in zip(_PASSWORD_SPECS, generated_passwords, strict=True):
    ...
    lines.extend(
        f"{key}={value}"
        for key, value in zip(_HMAC_KEYS, generated_keys, strict=True)
    )
...
def main(
    ...
    """Provision the reviewed values and report only the resulting state."""

    target = env_path or Path(__file__).resolve().parents[1] / ".env"
    changed = ensure_day4_operations_env(target, token_factory, key_factory)
    if changed:
        print("Day 4 operation environment created")
    else:
        print("Day 4 operation environment already present")
