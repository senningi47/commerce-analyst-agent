...
SQL, validates every reference query against the live product database
(read-only DSN), and emits three JSONL files with row counts and result
digests (multiset scoring from commerce_agent.product_eval.scoring).
...
QUESTIONS: list[dict] = [
    # ── Development 30 ──────────────────────────────────────────────────────
    q("dev-01", "development", "2018 年每月（按 'YYYY-MM'）的订单数是多少？按月升序排列。",
      "SELECT to_char(order_purchase_timestamp, 'YYYY-MM') AS ym, COUNT(DISTINCT order_id) AS order_count FROM retail.orders WHERE order_purchase_timestamp >= '2018-01-01' AND order_purchase_timestamp < '2019-01-01' GROUP BY 1 ORDER BY 1", ["orders"]),
    ...
    q("dev-06", "development", "全部订单的整体平均评价分是多少？按唯一 review_id 去重后计算。",
      "SELECT AVG(review_score) AS avg_review_score FROM (SELECT DISTINCT ON (review_id) review_id, review_score FROM retail.order_reviews) r", ["order_reviews"]),
    ...
    q("dev-10", "development", "2017 年每月（'YYYY-MM'）的支付金额（payment_amount）是多少？按月升序。",
      "SELECT to_char(o.order_purchase_timestamp, 'YYYY-MM') AS ym, SUM(pay.payment_value) AS payment_amount FROM retail.order_payments pay JOIN retail.orders o ON o.order_id = pay.order_id WHERE o.order_purchase_timestamp >= '2017-01-01' AND o.order_purchase_timestamp < '2018-01-01' GROUP BY 1 ORDER BY 1", ["orders", "order_payments"]),
    ...
    q("dev-15", "development", "已交付订单从下单到客户签收的平均天数是多少？（只统计有签收时间的订单，按日期差计算）",
      "SELECT AVG((order_delivered_customer_date::date - order_purchase_timestamp::date)) AS avg_delivery_days FROM retail.orders WHERE order_status = 'delivered' AND order_delivered_customer_date IS NOT NULL", ["orders"]),
    q("dev-16", "development", "商品目录中共有多少个产品（products 行数）？",
      "SELECT COUNT(*) AS product_count FROM retail.products", ["products"]),
    q("dev-17", "development", "各英文产品类别的平均评价分前 10 名是多少？（评价按 review_id 去重）",
      "SELECT t.product_category_name_english, AVG(r.review_score) AS avg_review_score FROM (SELECT DISTINCT ON (review_id) review_id, review_score, order_id FROM retail.order_reviews) r JOIN retail.orders o ON o.order_id = r.order_id JOIN retail.order_items oi ON oi.order_id = o.order_id JOIN retail.products p ON p.product_id = oi.product_id JOIN retail.product_category_name_translation t ON t.product_category_name = p.product_category_name GROUP BY 1 ORDER BY 2 DESC LIMIT 10", ["order_reviews", "orders", "order_items", "products", "product_category_name_translation"]),
    q("dev-18", "development", "卖家维度商品额前 10 名（含 seller_id）是多少？",
      "SELECT s.seller_id, SUM(oi.price) AS item_amount FROM retail.order_items oi JOIN retail.sellers s ON s.seller_id = oi.seller_id GROUP BY 1 ORDER BY 2 DESC LIMIT 10", ["sellers", "order_items"]),
    ...
    q("dev-20", "development", "一次性付清（分期数 = 1）的支付金额占总支付金额的比例是多少？",
      "SELECT SUM(payment_value) FILTER (WHERE payment_installments = 1) / SUM(payment_value) AS single_installment_share FROM retail.order_payments", ["order_payments"]),
    q("dev-21", "development", "运费与商品额比值（SUM(freight_value) / SUM(price)）最高的前 10 个英文产品类别是多少？（只统计商品额大于 0 的类别）",
      "SELECT t.product_category_name_english, SUM(oi.freight_value) / SUM(oi.price) AS freight_to_item_ratio FROM retail.order_items oi JOIN retail.products p ON p.product_id = oi.product_id JOIN retail.product_category_name_translation t ON t.product_category_name = p.product_category_name GROUP BY 1 HAVING SUM(oi.price) > 0 ORDER BY 2 DESC LIMIT 10", ["order_items", "products", "product_category_name_translation"]),
    ...
    q("dev-27", "development", "评论内容非空（review_comment_message 去除空白后非空）的评论行数是多少？",
      "SELECT COUNT(*) AS commented_reviews FROM retail.order_reviews WHERE NULLIF(btrim(review_comment_message), '') IS NOT NULL", ["order_reviews"]),
    q("dev-28", "development", "卖家维度平均评价分前 10 名（含 seller_id，评价按 review_row_id 去重）是多少？",
      "SELECT s.seller_id, AVG(r.review_score) AS avg_review_score FROM (SELECT DISTINCT ON (review_row_id) review_row_id, review_score, order_id FROM retail.order_reviews) r JOIN retail.orders o ON o.order_id = r.order_id JOIN retail.order_items oi ON oi.order_id = o.order_id JOIN retail.sellers s ON s.seller_id = oi.seller_id GROUP BY 1 ORDER BY 2 DESC LIMIT 10", ["sellers", "order_items", "orders", "order_reviews"]),
    q("dev-29", "development", "平均每个订单包含多少个订单项（order_items 行数 / distinct order_id）？",
      "SELECT COUNT(*)::numeric / COUNT(DISTINCT order_id) AS avg_items_per_order FROM retail.order_items", ["order_items"]),
    ...
    q("reg-02", "regression", "每个年度（'YYYY'）商品额最高的英文产品类别及其金额是什么？",
      "SELECT ym, product_category_name_english, item_amount FROM (SELECT to_char(o.order_purchase_timestamp, 'YYYY') AS ym, t.product_category_name_english, SUM(oi.price) AS item_amount, ROW_NUMBER() OVER (PARTITION BY to_char(o.order_purchase_timestamp, 'YYYY') ORDER BY SUM(oi.price) DESC) AS rn FROM retail.order_items oi JOIN retail.orders o ON o.order_id = oi.order_id JOIN retail.products p ON p.product_id = oi.product_id JOIN retail.product_category_name_translation t ON t.product_category_name = p.product_category_name GROUP BY 1, 2) ranked WHERE rn = 1 ORDER BY ym", ["orders", "order_items", "products", "product_category_name_translation"]),
    q("reg-03", "regression", "各客户州的平均送达天数（只统计已交付且有签收时间）降序排列是多少？",
      "SELECT c.customer_state, AVG((o.order_delivered_customer_date::date - o.order_purchase_timestamp::date)) AS avg_delivery_days FROM retail.orders o JOIN retail.customers c ON c.customer_id = o.customer_id WHERE o.order_status = 'delivered' AND o.order_delivered_customer_date IS NOT NULL GROUP BY 1 ORDER BY 2 DESC", ["customers", "orders"]),
    ...
    q("reg-05", "regression", "商品额前 10 名卖家的合计商品额占总商品额的比例是多少？",
      "SELECT SUM(top10.item_amount) / (SELECT SUM(price) FROM retail.order_items) AS top10_seller_share FROM (SELECT seller_id, SUM(price) AS item_amount FROM retail.order_items GROUP BY 1 ORDER BY 2 DESC LIMIT 10) top10", ["order_items"]),
    q("reg-06", "regression", "按季度（订单下单时间 'YYYY-QQ'）的平均评价分趋势是多少？（评价按 review_row_id 去重）",
      "SELECT to_char(o.order_purchase_timestamp, 'YYYY-QQ') AS yq, AVG(r.review_score) AS avg_review_score FROM (SELECT DISTINCT ON (review_row_id) review_row_id, review_score, order_id FROM retail.order_reviews) r JOIN retail.orders o ON o.order_id = r.order_id GROUP BY 1 ORDER BY 1", ["order_reviews", "orders"]),
    ...
    q("reg-08", "regression", "每个年度（'YYYY'）各支付方式的支付金额是多少？",
      "SELECT to_char(o.order_purchase_timestamp, 'YYYY') AS y, pay.payment_type, SUM(pay.payment_value) AS payment_amount FROM retail.order_payments pay JOIN retail.orders o ON o.order_id = pay.order_id GROUP BY 1, 2 ORDER BY 1, 2", ["orders", "order_payments"]),
    q("reg-09", "regression", "在商品额超过 50000 的英文类别中，运费与商品额比值最高的前 10 个是多少？",
      "SELECT t.product_category_name_english, SUM(oi.freight_value) / SUM(oi.price) AS freight_to_item_ratio FROM retail.order_items oi JOIN retail.products p ON p.product_id = oi.product_id JOIN retail.product_category_name_translation t ON t.product_category_name = p.product_category_name GROUP BY 1 HAVING SUM(oi.price) > 50000 ORDER BY 2 DESC LIMIT 10", ["order_items", "products", "product_category_name_translation"]),
    q("reg-10", "regression", "复购客户（同一 customer_unique_id 对应多个 customer_id）占全部 distinct customer_unique_id 的比例是多少？",
      "SELECT COUNT(DISTINCT customer_unique_id) FILTER (WHERE cnt > 1)::numeric / COUNT(DISTINCT customer_unique_id) AS repeat_customer_share FROM (SELECT customer_unique_id, COUNT(DISTINCT customer_id) AS cnt FROM retail.customers GROUP BY 1) c", ["customers"]),
    # ── Closed 10 ───────────────────────────────────────────────────────────
    q("closed-01", "closed", "2017 年每月（'YYYY-MM'）的商品额是多少？按月升序。",
      "SELECT to_char(o.order_purchase_timestamp, 'YYYY-MM') AS ym, SUM(oi.price) AS item_amount FROM retail.order_items oi JOIN retail.orders o ON o.order_id = oi.order_id WHERE o.order_purchase_timestamp >= '2017-01-01' AND o.order_purchase_timestamp < '2018-01-01' GROUP BY 1 ORDER BY 1", ["orders", "order_items"]),
    ...
    q("closed-10", "closed", "2018 年每季度（'YYYY-QQ'）的支付金额是多少？按季度升序。",
      "SELECT to_char(o.order_purchase_timestamp, 'YYYY-QQ') AS yq, SUM(pay.payment_value) AS payment_amount FROM retail.order_payments pay JOIN retail.orders o ON o.order_id = pay.order_id WHERE o.order_purchase_timestamp >= '2018-01-01' AND o.order_purchase_timestamp < '2019-01-01' GROUP BY 1 ORDER BY 1", ["orders", "order_payments"]),
]
...
def main() -> int:
    ...
    with psycopg.connect(dsn, connect_timeout=10) as conn:
        ...
        for item in QUESTIONS:
            ...
            for table in item["gold_tables"]:
                exists = cur.execute  # noqa: F841 - placeholder, checked below
                with conn.cursor() as cur:
                    cur.execute(
    ...
    if failures:
        ...
        for failure in failures:
            print(" ", failure)
        return 1
    print(f"all {len(QUESTIONS)} reference queries validated")
    return 0
